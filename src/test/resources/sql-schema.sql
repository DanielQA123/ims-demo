...
create table if not exists ims.orderline(id int primary key auto_increment, item_id int, order_id int not null, item_name varchar(100), foreign key (item_id) references items (id), foreign key (order_id) references orders (id));