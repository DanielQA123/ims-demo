...
create table if not exists ims.orderline(id int primary key auto_increment, item_id int not null, order_id int not null, foreign key (item_id) references items (id), foreign key (order_id) references orders (id));